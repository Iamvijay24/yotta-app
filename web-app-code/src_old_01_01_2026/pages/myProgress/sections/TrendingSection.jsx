...
const Trending_section = () => {
  ...
  const navState = {
    from: 'trending',
    fromTitle: 'Top Trending Courses',
    returnTo: '/trending-courses'
  };
  // Memoized enrolledIds
  ...
  if (dashboardStatus === 'failed' || enrolledStatus === 'failed') {
    return (
      <PageContainer>
        <BreadcrumbComponent style={{ margin: '0 40px' }}/>
        <Empty description={dashboardError || enrolledError || 'Failed to load trending courses'} style={{ padding: '70px 0' }}/>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer className={styles.sectionPageContainer}>
      ...
      <HeaderSection className={styles.headerSection}>
        <Text className={styles.resultsText}>Top Trending Courses</Text>
        <CourseFilter
          courses={allCourses}
      ...
      {filteredCourses.length === 0 ? (
        <Empty description="No trending courses found" />
      ) : (
        <>
