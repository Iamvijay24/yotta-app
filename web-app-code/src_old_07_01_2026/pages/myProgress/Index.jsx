...
const MyProgress = () => {
    ...
    const fetchProgress = async () => {
      ...
      try {
        ...
        if (!err.response) {
          ...
          message.error(err.response.data?.message || 'Failed to fetch course progress');
        }
      }
    };
  ...
  if (dashboardStatus === 'failed' || enrolledStatus === 'failed') {
    return (
      <PageContainer>
        <Empty description={dashboardError?.message || enrolledError?.message || 'Failed to load courses'} />
      </PageContainer>
    );
  }
  return (
