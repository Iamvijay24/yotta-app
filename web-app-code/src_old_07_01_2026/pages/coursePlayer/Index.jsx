...
const CoursePlayer = () => {
  ...
  useEffect(() => {
    ...
    if (rawError && !usingFallback && hasAttemptedFetch) {
      if (import.meta.env.DEV) console.log('Applying fallback data due to API error');
  ...
  useEffect(() => {
    ...
    if (!loading && !hasAttemptedFetch) {
      if (import.meta.env.DEV) console.log('Attempting to fetch course structure for:', courseId);
      setHasAttemptedFetch(true);
      dispatch(fetchCourseStructure(courseId));
    }
  }, [courseId, hasAttemptedFetch, rawCourseStructure, rawError, loading, dispatch]); // Added raw deps to prevent stale closures
  ...
  useEffect(() => {
    if (rawError && hasAttemptedFetch && !rawCourseStructure && !usingFallback) {
      ...
      if (rawError?.status === 404 || rawError?.message === 'Course not found') {
        if (import.meta.env.DEV) console.log('Navigating to /not-found due to 404 error');
  ...
  useEffect(() => {
    ...
    if (courseStructure || usingFallback) {
      if (import.meta.env.DEV) console.log('Course structure loaded successfully or fallback applied');
      setLocalLoading(false);
    } else if (rawError && hasAttemptedFetch) {
      if (import.meta.env.DEV) console.log('Stopping loading due to error');
      setLocalLoading(false);
    } else if (!loading && hasAttemptedFetch && !rawCourseStructure && !rawError) {
      // Edge case: loading finished but no data and no error
      if (import.meta.env.DEV) console.log('Loading finished with no data');
      setLocalLoading(false);
    }
  ...
  const durations = useMemo(() => rawDurations, [rawDurations]);
